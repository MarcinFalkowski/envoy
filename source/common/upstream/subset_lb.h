...
private:
  ...
  HostConstSharedPtr chooseHostForSelectorFallbackPolicy(
      const SubsetSelectorFallbackParams& fallback_params,
  ...
  public:
    ...
    const Router::MetadataMatchCriteria* metadataMatchCriteria() override { return metadata_match_.get(); }
    const Network::Connection* downstreamConnection() const override { return wrapped_->downstreamConnection(); }
    const Http::HeaderMap* downstreamHeaders() const override { return wrapped_->downstreamHeaders(); }
    ...
    bool shouldSelectAnotherHost(const Host& host) override { return wrapped_->shouldSelectAnotherHost(host); }
