...
void LoadBalancerSubsetInfoImpl::validateSelector(const SubsetSelector& selector) const {
  if (selector.fallback_policy_ != envoy::api::v2::Cluster::LbSubsetConfig::LbSubsetSelector::KEYS_SUBSET) {
  ...
  if (!std::includes(
      selector.selector_keys_.begin(), selector.selector_keys_.end(),
      selector.fallback_keys_subset_.begin(), selector.fallback_keys_subset_.end())) {
